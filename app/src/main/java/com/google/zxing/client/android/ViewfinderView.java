...
public final class ViewfinderView extends View {
	...
	private void drawTextInfo(Canvas canvas, Rect frame) {
		...
		// 文字在扫描框上方
		// canvas.drawText(labelText, frame.left + frame.width() / 2, frame.top
		// - CORNER_RECT_HEIGHT, paint);
		// 文字在扫描框下方
	...
	private void drawLaserScanner(Canvas canvas, Rect frame) {
		...
		// canvas.drawRect(frame.left + 2, middle - 1, frame.right - 1, middle +
		// 2, paint);
		// 线性渐变
		LinearGradient linearGradient = new LinearGradient(frame.left, scannerStart, frame.left,
				scannerStart + SCANNER_LINE_HEIGHT, shadeColor(laserColor), laserColor, Shader.TileMode.MIRROR);
		...
		if (scannerStart <= scannerEnd) {
			// 矩形
			// canvas.drawRect(frame.left, scannerStart, frame.right,
			// scannerStart + SCANNER_LINE_HEIGHT, paint);
			// 椭圆
