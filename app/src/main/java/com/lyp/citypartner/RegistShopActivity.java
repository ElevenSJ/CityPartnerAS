...
public class RegistShopActivity extends BaseActivity {
	...
	private Handler mainHandler = new Handler() {
		public void handleMessage(android.os.Message msg) {
			switch (msg.what) {
			case MessageContants.MSG_REGIST: {
				if (mWaitDialog == null) {
					mWaitDialog = new WaitDialog(RegistShopActivity.this, R.string.loading_data);
				}
				if (!isPause) {
				    mWaitDialog.show();
				    parseRegistInfo((String) msg.obj);
				}
				break;
			}
	...
	public void getVerifyCode(View view) {
		...
		if (phoneET == null) {
			ToastUtil.showLong(RegistShopActivity.this, "号码不能为空");
			return;
		}
...
//		ToastUtil.showLong(RegistShopActivity.this, "接口开发中...");
	...
	public void regist(View view) {
		...
		NetProxyManager.getInstance().toRegist(mainHandler, phone, verifyCodeET.toString(), usernameET.toString());
//		NetProxyManager.getInstance().toLoginByPWD(mainHandler, phone, verifyCodeET.toString());
	}
	...
	private void parseSMSAuthCodeInfo(String result) {
		...
		try {
			...
			ToastUtil.showLong(RegistShopActivity.this, "短信验证码发送成功，请等待!");
		} catch (Exception ex) {
	 ...
	 */
	private void parseRegistInfo(String result) {
		if (Errors.ERROR_NET.equals(result) || Errors.ERROR_SERVER.equals(result)) {
			ToastUtil.showLong(this, R.string.network_error);
			if (mWaitDialog != null && mWaitDialog.isShowing()) {
				mWaitDialog.dismiss();
